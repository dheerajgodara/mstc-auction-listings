import type { AuctionRecord, LotRecord } from "@/types/auction";
...
function lotQuantityMt(lot: LotRecord): number | null {
  ...
  const blob = [lot.item_title, lot.item_description, lot.lot_description_text, lot.lot_details_text]
    .filter(Boolean)
    .join(" ");
...
export function formatMt(value: number): string {
  if (value >= 100) return value.toLocaleString("en-IN", { maximumFractionDigits: 0 });
  if (value >= 10) return value.toLocaleString("en-IN", { maximumFractionDigits: 1 });
...
function classifyMaterial(
  ...
  const patterns: [RegExp, DisplayMaterialCategory][] = [
    [/tower|transmission|earth\s*wire|acsr|conductor|moose|panther|deer/i, "transmission_scrap"],
  ...
  if (assetCategory === "vehicle") return { key: "vehicle_lot", label: "Vehicle lot" };
  if (assetCategory === "machinery") return { key: "machinery", label: "Machinery" };
  if (assetCategory === "coal") return { key: "coal", label: "Coal" };
  if (assetCategory === "minerals") return { key: "minerals", label: "Minerals" };
  if (assetCategory === "property") return { key: "property", label: "Property" };
  return { key: "other", label: "Other" };
}

function shortMaterialLabel(category: DisplayMaterialCategory, blob: string): string {
  if (category === "transmission_scrap") {
    ...
    if (/conductor|acsr|moose|panther|deer/i.test(blob)) return "Conductor Scrap";
...
export function normalizeLocation(
  ...
  if (!city && rawClean) {
    const parts = rawClean.split(",").map((p) => p.trim()).filter(Boolean);
    for (const part of parts) {
      ...
      if (part.length >= 3 && !/\b(kv|sub.?station|depot|yard|site)\b/i.test(pl)) {
  ...
  if (city && inferredState) return { city, state: inferredState, raw: rawClean || null, confidence: "high" };
  if (city) return { city, state: inferredState, raw: rawClean || null, confidence: "medium" };
  if (inferredState && rawClean) return { city: null, state: inferredState, raw: rawClean, confidence: "low" };
  if (rawClean) return { city: null, state: null, raw: rawClean, confidence: "low" };
  return { city: null, state: null, raw: null, confidence: "low" };
}

function buildQuantitySummary(lots: LotRecord[]): { summary: string | null; totalMt: number | null } {
  ...
  if (hasMt && lotParts.length) {
    ...
    if (lotParts.length <= 3) {
      const detail = lotParts.map(({ label, mt }) => `${formatMt(mt)} MT ${label}`).join(" · ");
      return { summary: `${lots.length} lots · ${detail}`, totalMt };
    }
    const top = [...lotParts].sort((a, b) => b.mt - a.mt).slice(0, 3);
    const detail = top.map(({ label, mt }) => `${formatMt(mt)} MT ${label}`).join(" · ");
    return {
...
function buildDisplayTitle(
  ...
  if (summary && summary.length <= 90 && !summary.toLowerCase().startsWith("bids are invited")) {
    return summary;
  }
...
export function auctionTotalMt(auction: AuctionRecord): number | null {
  const enriched = enrichAuctionDisplay(auction);
  if (enriched.display_total_quantity_mt != null) return enriched.display_total_quantity_mt;
  let total = 0;
  let found = false;
