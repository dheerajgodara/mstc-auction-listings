...
function LotSection({
  ...
  return (
    <div>
      ...
      <div className="mt-1">
        <ExpandableText text={text} previewLen={700} />
      </div>
    </div>
  );
}
...
function LotCard({ lot, index }: { lot: LotRecord; index: number }) {
  ...
  const parameters = getLotSectionDisplayText(lot, "lot_parameters_text");
  const other = getLotSectionDisplayText(lot, "lot_other_details_text");
  const documentsText = getLotSectionDisplayText(lot, "lot_documents_text");
  ...
  return (
    ...
    >
      ...
      <dl className="grid gap-3 sm:grid-cols-2">
        ...
        <LotField label="Product type" value={lot.product_type} />
        <LotField
          label="Bid increment"
          value={
            lot.bid_increment != null
              ? `₹${Number(lot.bid_increment).toLocaleString("en-IN")}`
              : null
          }
        />
        <LotField
      ...
      <LotSection heading="Description" text={description} />
      {details !== description && <LotSection heading="Details" text={details} />}
      <LotSection heading="Parameters" text={parameters} />
      <LotSection heading="Other details" text={other} />
      <LotSection heading="Documents noted" text={documentsText} />
  ...
  return (
    <section aria-labelledby="lots-heading" className="space-y-4">
      ...
      <div className="space-y-4">
        {lots.map((lot, index) => (
          <LotCard key={lot.lot_id || index} lot={lot} index={index} />
        ))}
      </div>
    </section>
  );
